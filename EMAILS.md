...
כל 16 המיילים נרשמים ב-`EmailLog` - כולל דיווחי התקלה וההודעות הפנימיות, שעד 2026-09-22 לא
נרשמו בכלל. המסך עצמו הועבר ב-2026-09-23 מ-`app/management/email-logs` (נגיש להנהלה ראשית +
מתכנת) אל `app/admin/site-settings/email-logs` - נגיש רק דרך הגדרות ← הגדרות אתר ← טאב "מיילים",
מוגבל למתכנת בלבד (`DEVELOPER_ONLY_ROLES`) גם ברמת הדף וגם ברמת ה-API (`checkAuth('מתכנת')`
ב-`app/api/email-logs/route.js`).
