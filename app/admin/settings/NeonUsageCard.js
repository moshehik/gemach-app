...
export default function NeonUsageCard() {
  ...
  const tile = (icon, title, value, sub) => (
    <div style={{ flex: '1 1 160px', background: 'var(--element-bg)', border: '1px solid var(--element-border)', borderRadius: '12px', padding: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '0.4rem' }}>
        {icon}
        {title}
      </div>
      <div style={{ fontSize: '1.4rem', fontWeight: '700', color: 'var(--text-main)' }}>{value}</div>
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '2.5rem 0', color: 'var(--text-muted)' }}>
  ...
  return (
    <div style={{ padding: '1.25rem 0', borderBottom: '1px solid var(--element-border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        <button type="button" onClick={fetchUsage} title="רענון נתונים"
          style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'var(--card-bg)', border: '2px solid var(--element-border)', color: '#3b82f6', borderRadius: '10px', padding: '0.5rem 1rem', cursor: 'pointer', fontWeight: '600' }}>
          <RefreshCw size={16} />
          רענן
        </button>
      </div>
      ...
      <div style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ height: '8px', background: 'var(--element-border)', borderRadius: '4px', overflow: 'hidden' }}>
          <div style={{ width: `${freePct}%`, height: '100%', background: freePct > 85 ? '#ef4444' : freePct > 60 ? '#f59e0b' : '#22c55e', transition: 'width 0.4s' }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', color: 'var(--text-muted)', margin: '0.7rem 0 0.3rem' }}>
      ...
      <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', background: 'var(--element-bg)', border: '1px solid var(--element-border)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '0.75rem' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
        <thead>
          <tr style={{ color: 'var(--text-muted)', textAlign: 'right' }}>
            <th style={{ padding: '0.4rem', borderBottom: '2px solid var(--element-border)' }}>סביבה (ענף)</th>
            <th style={{ padding: '0.4rem', borderBottom: '2px solid var(--element-border)' }}>מצב</th>
            <th style={{ padding: '0.4rem', borderBottom: '2px solid var(--element-border)' }}>Autoscaling (CU)</th>
          </tr>
        </thead>
        <tbody>
          {endpoints.map(e => (
            <tr key={e.id}>
              <td style={{ padding: '0.4rem', borderBottom: '1px solid var(--element-border)', fontWeight: '600' }}>{e.branch}</td>
              <td style={{ padding: '0.4rem', borderBottom: '1px solid var(--element-border)' }}>
                <span style={{ color: e.state === 'idle' ? 'var(--text-muted)' : '#16a34a' }}>
