...
const ROWS = [
  ...
  { id: 3, req: 'הוראת קבע וגביה ממאחרים (עד 19:00)', key: 'hok_enabled / hok_auto_charge_enabled / hok_auto_charge_hour / hok_charge_amount', cat: 'הוראת קבע', type: 'booleans+שעה+סכום', def: 'כבוי / כבוי / 19:00 / ריק=מחיר מקורי, 0=ללא גביה', where: 'הגדרות מערכת ← הוראת קבע (4 שדות)' },
  ...
  { id: 22, req: 'שליחת מייל לפי תאריך אירוע + מעקב אישורים', key: 'bulk_email_by_event_date', cat: 'אוטומציה', type: 'toggle', def: 'כבוי', where: 'הגדרות מערכת ← אוטומציה ← "שליחת מייל לפי תאריך אירוע"' },
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      ...
      <div className="card card-pad" style={{ marginTop: 16 }}>
        <h3 style={{ margin: '0 0 8px' }}>סטטוס מימוש (07/09/2026, אחרי סבב תיקונים שני)</h3>
        <ul style={{ margin: 0, paddingInlineStart: 18, lineHeight: 1.7, fontSize: 13, color: 'var(--text-2)' }}>
          <li>✅ <strong>גמור ומחובר בפועל (נבדק מול הקוד עצמו, לא רק לפי הכוונה):</strong> 1 ציפוף מוסתר, 3 הו״ק (כולל ללקוח קיים, לא רק לקוח חדש) - Obligation בלבד, 4 חובה קשיחה + מייל/כתובת/דיוור, 5 מייל יצירה, 6 תזכורת מחר, 7 ללא תיקונים, 8 דוח יומי, 9 מאחרים, 12 תפוצה (לוג פנימי - אין ספק חיצוני), 13 טלפוני + סניף (התיקון: המתג של 13 שולט עכשיו על שדה 13 בפועל, לא היה מחובר קודם), 14 ת״ז, 16 מיון אחרונים, 17 חסימת 6, 18 טבלת טווח (רק קדימה בזמן, לא כולל תאריכים שהיו), 19 משלוח יום לפני - עכשיו באמת זז תאריך המשלוח פר-הזמנה (קודם היה צ׳קבוקס מת), 20 מיון הדפסה משלוח/רגיל - הלוגיקה נכתבה ועובדת, אך <strong>אין כרגע כפתור בממשק שמדפיס כמה הזמנות יחד</strong> - לכן בפועל לא ניתן להפעיל עדיין (ר׳ הערה למטה), 21 סימון שמלה חסרה בהדפסה - עכשיו באמת בודק מלאי ומחפש החזרה למחר, 22 מייל לפי תאריך אירוע + מעקב אישורים אמיתי (קישור אישור אישי בכל מייל, ברירת מחדל הוחזרה לכבוי), 23 פגומה, 24/25 הודעות משמרת/הנהלה - נבנה ממשק אמיתי (קודם היה הערת קוד בלבד), 26 ברקוד לא תקין ← רשימה + טופל, 27 עריכת מושכר חלקי, 29 התראה בכניסה, 30 כובסת ביציאה, 31 הקלדה ידנית כפולה + חתימה - עכשיו יש ממשק אמיתי בכרטיס ההשכרה (קודם השרת תמך אך שום מסך לא קרא לו), 34 סניפים, 36 שינוי קטגוריית דגם, 37 לא-נלקחו, 38 עמודות ביטול.</li>
          <li>🔶 <strong>מחובר אך חסר תוכן/נתונים כדי לפעול בפועל:</strong> 2 רינת לב - הוחלט: מוסד נפרד לגמרי, הקוד מוכן אך שדה קוד המוסד עדיין ריק, 15 משלוח - הלוך/חזור ומחיר-לפי-עיר מחוברים אך <em>אין טבלת מחירים</em> (דילגת על השאלה בינתיים), 20 (ר׳ למעלה - חסר כפתור UI), 32/33 קיוסק - רישום לקוח וחיפוש דגם אמיתיים ועובדים, אבל <em>כפתור ההזמנה העצמאית מנוטרל בכוונה</em> עד שתוחלט מדיניות אישור/תשלום, 35 פרימיום - התמחור עצמו תוקן ועובד, אך <em>אין עדיין שורת מחירון בפועל</em> לקטגוריית פרימיום/יוקרה - יש להזין במחירון כדי שיהיה אפקט.</li>
          <li>⏳ <strong>דילוג לפי בקשתך המפורשת:</strong> 10/11 ימות המשיח (שלד מבודד, לא פעיל, ללא URL/טוקן).</li>
          <li>⏳ <strong>עדיין שלד בלבד (לא נגענו בסבב הזה):</strong> 28 דגם מפוצל ל-2 חלקים - יש שדה `isSplit` וסימון בכרטיס הדגם, אבל אין עדיין אכיפת 2 ברקודים נפרדים בהשכרה/החזרה.</li>
          <li>✅ לא נמחקו פיצ׳רים - כל &quot;ביטול/הסתרה&quot; הוא toggle. כבוי = חוזר מיד.</li>
        </ul>
      </div>
      ...
      <div className="card card-pad" style={{ marginTop: 16 }}>
        ...
        <ul style={{ margin: 0, paddingInlineStart: 18, lineHeight: 1.7, fontSize: 13, color: 'var(--text-2)' }}>
          ...
          <li><strong>#20 - הפער שנשאר:</strong> לוגיקת המיון (משלוח/רגיל) נכתבה ב-<code>app/print/order/page.js</code> ותומכת בהדפסת כמה הזמנות יחד (`?orderId=1,2,3`), אך אין כרגע שום מקום בממשק שבפועל שולח כמה הזמנות בבת אחת להדפסה - כל הדפסה היום היא הזמנה בודדת. כדי שהפיצ׳ר יהיה שימושי בפועל צריך כפתור "הדפס דפי הכנות" מרוכז (למשל מתוך רשימת ההזמנות היומית) - זו עבודה נוספת קטנה, לא סגורה עדיין.</li>
          <li><strong>#35 - הפער שנשאר:</strong> התמחור עצמו תקין, אך צריך שהמנהלת תוסיף בפועל שורת מחיר בקטגוריה "פרימיום"/"יוקרה" במחירון (כרטיס מחירון) - בלי זה המתג לא ישפיע כי אין ממה לגזור מחיר.</li>
          <li><strong>מיגרציית DB:</strong> 3 עמודות חדשות (Order.deliveryOneDayBefore, Notification.category/handledAt/handledById, EmailLog.confirmToken/acknowledgedAt) הופעלו ישירות מול ה-DB החי (תוספתי בלבד, ללא מחיקת נתונים) - ר׳ סיכום השיחה.</li>
      ...
      <div className="card card-pad" style={{ marginTop: 16, borderRight: '4px solid var(--warning)' }}>
        <h3 style={{ margin: '0 0 8px' }}>עדיין ממתין לתשובתך (לא שולב עד שתאשר)</h3>
        <ol style={{ margin: 0, paddingInlineStart: 18, lineHeight: 1.7, fontSize: 13 }}>
          <li><strong>#28 דגם 2 חלקים</strong> - האם זה דגם אב עם 2 דגמי-בן, או שדה כמות=2 על אותו דגם? האם התמחור הוא לסט או פר-חלק? האם 2 ברקודים נפרדים במלאי?</li>
          <li><strong>#32-33 עמדת לקוח</strong> - האם לקוח יוצר הזמנה <em>מאושרת</em> או טיוטה שממתינה לאישור עובד? האם נדרש תשלום מראש באתר? (הרישום והחיפוש כבר עובדים בפועל - רק ההזמנה עצמה מחכה)</li>
          <li><strong>#34 סניפים</strong> - רשימת סניפים סופית? האם מלאי משותף או מופרד פר-סניף?</li>
        </ol>
        <p style={{ margin: '10px 0 0', fontSize: 12, color: 'var(--text-3)' }}>נענו וטופלו: #2 (מוסד נפרד), #3/23 (נשאר כהתחייבות בלבד ללא חיוב אוטומטי), #10-11 (דילוג), #15 (דילוג בינתיים על טבלת המחירים).</p>
      </div>
    </div>
  );
}
