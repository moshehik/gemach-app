  ...
  return (
    <div className="container animate-fade-in" style={{ paddingTop: '2.5rem', paddingBottom: '4rem', maxWidth: '860px', margin: '0 auto' }}>
      ...
      <Section title="מגבלה ידועה: תשלומים ופריטים של הזמנות קיימות">
        <Callout tone="warn" title="עודכן 06.08.2026">
          שורות של פריטי הזמנה, תשלומים וחיובים מוכנסות לכל הזמנה <strong>שעדיין אין לה אף שורה כזו</strong> —
          בין אם ההזמנה חדשה למסד באותה הרצה ובין אם היא הזמנה ישנה שנשארה בלי פרטים (למשל בגלל הרצה שנקטעה
          באמצע). אם להזמנה <strong>כבר יש</strong> ולו שורת תשלום/פריט/חיוב אחת — הכלי לא נוגע בה, גם אם באקסס
          נוסף לה תשלום או פריט חדש.
        </Callout>
        <p style={{ lineHeight: 1.8, marginBottom: '0.75rem' }}>
          עד 06.08.2026 הבדיקה הייתה "האם ההזמנה חדשה למסד בהרצה הזו" — מה שיצר פער אם התהליך נקטע בין שמירת
          ההזמנה לשמירת פרטיה (בדיוק המקרה שיצר את הפער של 2,035 ההזמנות המתועד למטה). עכשיו הבדיקה היא ישירה
          מול המסד: אילו הזמנות באמת חסרות את כל שלוש הטבלאות, ללא תלות בהרצה הנוכחית.
        </p>
        <p style={{ lineHeight: 1.8, marginBottom: '0.75rem' }}>
          המגבלה שנשארה: בהעברה המקורית מאקסס למערכת, שלוש הטבלאות האלה קיבלו מספרי <Code>legacyId</Code>{' '}
          <strong>מומצאים</strong> (רצים סדרתית, ללא קשר למספרי האקסס האמיתיים). אין דרך בטוחה להתאים שורה
          קיימת כזו לשורת האקסס המקורית שלה, ולכן הכנסה עיוורת להזמנה שכבר יש לה נתונים עלולה לרשום תשלום
          פעמיים — ולכן הזמנות כאלה עדיין לא נסרקות מחדש אוטומטית.
        </p>
        <ul style={{ lineHeight: 1.9, paddingRight: '1.2rem' }}>
