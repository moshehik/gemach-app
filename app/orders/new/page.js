...
export default function NewOrderPage() {
  ...
  const activeItems = (order.items || []).filter(i => !i.isDeleted);
  const datesFilled = (order.isAbroad || order.isWeekdayEvent) ? (order.fromDate && order.toDate) : order.eventDate;
  // 9090b43a - עיר משלוח נבחרת מתוך רשימת הערים שיש להן מחיר מוגדר ב-delivery_price_by_city
  // (אותה הגדרה ששלוב במחיר המשלוח בפועל, ר' lib/pricingEngine.js applyDeliveryCharge) -
  // כדי שלא יהיה אפשר לבחור עיר שאין לה בכלל מחיר משלוח מוגדר. אם אין הגדרה כזו (עדיין),
  // חוזרים לשדה טקסט חופשי כמו קודם.
  const deliveryCityOptions = (() => {
    try {
      return Object.keys(JSON.parse(settings.delivery_price_by_city || '{}')).sort();
    } catch {
      return [];
    }
  })();
                      ...
                      <div className="field">
                        ...
                        {deliveryCityOptions.length > 0 ? (
                          <select className="select" value={order.deliveryCity || ''} onChange={e => setOrder(prev => ({ ...prev, deliveryCity: e.target.value }))}>
                            ...
                            {deliveryCityOptions.map(city => <option key={city} value={city}>{city}</option>)}
                          </select>
                        ) : (
