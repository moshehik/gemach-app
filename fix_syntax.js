...
const targetRegex = /tr:nth-child\(even\) \{ background: #f8fafc; \}[\s\S]*?\.summary[\s\S]*?\`\}\} \/>/;

const replacementStr = `tr:nth-child(even) { background: #f8fafc; }
...
if (targetRegex.test(c)) {
  c = c.replace(targetRegex, replacementStr);
  fs.writeFileSync('app/customer-interface/page.js', c);
  console.log("Restored missing code with regex!");
} else {
