...
## תיקונים אחרי סקירה עצמאית (REVIEW-wizard-1, 2026-09-24)
- **בוחר המאשר מסונן לפי הרמה/המפתח** (M-1) — בדיוק כמו `PopupProvider.showAuthPrompt`: חסימת לקוח (`'הנהלה ראשית'`) = roleId 0/2 בלבד; `feature:*` = `employee.approvals[key]` (הנהלה ראשית/מתכנת תמיד). כל קריאה באב-הטיפוס מעבירה את המפתח שלה: `feature:missing_contact_approval` (לקוח קיים), `feature:special_spacing_approval`, `feature:past_date_order_approval`, `feature:payment_exit_approval`. מתקן את WZ-54/55/71/127/129 בתצוגה.
- **אופן התשלום של חיוב אשראי** (M-2, מחליף את ברירת המחדל של Q-W6): נרשם `payment.method` מהתפריט, כמו היום (אותו שדה, אותו מקור — בלי מחרוזת קבועה). "חיוב באשראי" קודם מעביר את התפריט לאופן האשראי הראשון ברשימת הארגון (`ALLOWED_PAYMENT_METHODS`), ולכן לא נרשם "מזומן" עם אישור חיוב (WZ-150). אם אין ברשימה אופן אשראי — נרשם מה שבתפריט (כמו היום).
- **עיר למשלוח** (M-3): חובה לפי `isDeliveryCityRequired` בדיוק — כשעיר הלקוח אינה מפתח ב-`delivery_price_by_city`; **טבלה ריקה = תמיד חובה** (WZ-81). האפשרות הראשונה היא "בחירת עיר" (אין אפשרות "כמו אצל הלקוח" — ערך ריק לא יוצר חיוב משלוח, WZ-108/`applyDeliveryCharge`), ורמז "בלי עיר למשלוח לא נוסף חיוב משלוח". המחיר ברמז = עיר → `delivery_price` → 50 (כמו `/api/orders/calculate`).
- **לקוח חדש בלי קבוצת חובה** (M-4): שגיאת שדה ("צריך למלא טלפון נוסף או מייל") ולא חלון אישור — כי `POST /api/customers` דוחה את זה תמיד (WZ-155); היום האישור מוצג ואז השמירה נכשלת ב-400. לקוח **קיים** חסר קבוצה — עדיין חלון אישור `feature:missing_contact_approval` (WZ-55).
- **הערת "יציאה באישור מנהל"** = אותו טקסט שנשלח היום: `יציאה באישור מנהל (סכום מבוקש: ₪X) | הערה` (WZ-128).
- **סניף ביצוע** מתמלא רק מהסניף האחרון שנבחר בעמדה (`gemach_last_order_branch`), לא מהסניף הראשון ברשימה (WZ-75).
- **מצבי שרת נוספים**: שמירה שהצליחה עם `warning` (WZ-11) = NoticeBar + הודעת שגיאה; 400 מ-`POST /api/customers` (למשל ת״ז קיימת) = שגיאה בשדה.
- **סטטוס תשלום ב-NoticeBar** = `calculatePaymentStatus` (4 הערכים בלבד).

## ליטוש על מעטפת הסקיצה (2026-09-24, C-1.14 / C-1.15 / C-1.16) — איפה כל פריט מוצג עכשיו
הלוגיקה לא השתנתה; השתנה רק איפה/איך המידע מוצג. אף פריט לא ירד.
- **סטפר** = רכיב ציר הזמן של הסקיצה (`.tlx > .tx.done|.cur|.fut`, `.dot` + `.ck`, `.tt b/span`). ערך הצומת (WZ-17) בשורת ה-span; **שלב נעול** מציג אייקון מנעול ואת הסיבה כטקסט בשורת ה-span (לא ⓘ), וגם בכרטיס הריחוף. קפיצה = אותן בדיקות של "המשך" (Q-W3).
- **ניווט השלבים** (WZ-20..25: המשך / חזרה / ביטול-יציאה / יצירת ההזמנה) עבר לפעולות של כרטיס הסיכום (`.rcard.cart .cart-actions`), מיד אחרי אריחי המבט-המהיר — כך הוא תמיד בראש העמודה בדסקטופ ובגיליון התחתון בטלפון. הסיבה לכפתור מושבת = שורת טקסט מתחת לכפתור.
- **תגיות שהוסרו → טקסט**: "טיוטה #N" (WZ-6) = טקסט בשורת הכותרת + כפתור אייקון "פתיחת הטיוטה בלשונית חדשה"; שם הלקוח = אריח בכרטיס הסיכום + ערך צומת 1; "בסל · N" = "בסל (N)"; "N מתוך max פריטים" = שורת טקסט (אזהרה בצבע כשהגיעו למקסימום); "חסומה" = טקסט+אייקון (`.stx.warn`). סטטוס התשלום (calculatePaymentStatus) = אריח `.gl.pay` בכרטיס הסיכום.
- **חיפוש לקוח בשם / בחירת דגם / עיר למשלוח** = רכיב ה-combobox של הסקיצה (`.cb`, כמו cityCombo). עיר למשלוח עדיין לפי אותם כללי חובה (M-3).
- **אישור מנהל** = חלון `askManagerApproval` של הסקיצה (`.apprwin`, 4 ספרות קוד עובד). הקוד מזהה את העובד; מי שלא מורשה לרמה/למפתח (M-1) נדחה עם "ל<שם> אין הרשאה לאשר את זה"; "אין מאשרים מורשים" = הודעה בחלון. **שינוי ייצוג**: היום זה בוחר-עובד + סיסמה מלאה (PopupProvider) — ראו שאלה בדוח.
- **תשלום** (WZ-115..117): סכום = `.amtin`, אופן תשלום = כפתורי `.methods` (אותה רשימה ממש, WZ-114), הערה בתוך מקטע מתקפל. חלון חיוב האשראי = חלון הזנת מידע בהיר (`.dlg.mailwin`), שדה אחד בשורה.
- **הצלחה** (WZ-9/12): חלון ההצלחה של הסקיצה (`.success`) + שורת "אחרי שמירה עוברים ל…" בעמוד; אזהרת שרת (WZ-11) = NoticeBar. שגיאות שרת = NoticeBar מסוג התראה; אישורים קצרים = toast של הסקיצה.
- **טפסים**: כל שדה בשורה משלו (`.fcol`) — שם פרטי/משפחה, טלפון/טלפון נוסף, עיר/רחוב/מספר בית, בנק/סניף/חשבון, תוקף/סכום/תשלומים.
