...
## 5. Amendments A7-A15 (2026-09-25, owner requests C-1.17/C-1.18/C-1.25..C-1.30)
A7 tokens (`--gold-rich-c/-h`, `--gold-rich`, `--gold-light`, `--card-bw`, `--card-line`, `--bg-dur`) · A8 card border 2px token (C-1.30) · A9 card header navy strip + navy icon on rich-gold circle (C-1.27) · A10 buttons: primary rich gold, all others light gold -> rich gold on hover, salmon only `.danger`/`.btn-pink` (C-1.29) · A11 `.lrow` animated row with the `.hf-o` gold bar (C-1.26) · A12 links `.lnk`/`.cpy`: navy, no underline, gold bar (C-1.18) · A13 inputs right-aligned (C-1.17) · A14 animated background changes, light-gold standing state of `.scan` + adjacent button (C-1.28) · A15 shared footer `.site-foot` + `mountSiteFooter(el,{groups,name,ver,date})` in `runtime-tail.src.js` (C-1.25).
**Parity note:** the card border/padding, card-h, button fills now intentionally differ from the sketch (owner-directed); parity-check reports those as expected deviations. Only `archetype-dashboard.html` has the new block; the other 6 prototypes still carry the old sha (`build-shell.cjs --check` fails for them until the cleanup round).
