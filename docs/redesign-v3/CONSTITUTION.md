...
| C-1.20 | **אין מלל מיותר — נוסף 2026-09-25 לבקשת הבעלים** (דוגמה מהפרופיל: מתחת למתג התראות פעיל/לא פעיל נכתב "עכשיו פעיל. נשמר יחד עם שאר הפרטים." — מלל שאין בו מידע, ונכנס שוב ושוב) | **כל שורת מלל חייבת להוסיף מידע שהמשתמש לא רואה כבר.** אסור: (1) שורת סטטוס שחוזרת על מצב שהפקד עצמו מראה (מתג פעיל/כבוי, "נשמר", "עכשיו X"); (2) הסבר להתנהגות מובנת מאליה או לשיטת השמירה כשאין בה שום חריג; (3) "סיכום" או משפט תיאור שנוסף לצד פיצ'ר; (4) משפטי מילוי, ברכות והרגעות; (5) כפילות בין תווית, ⓘ וטקסט משנה. מצב פקד מוצג **בפקד** (צבע/אייקון/תווית המתג), הסבר נחוץ = tooltip/ⓘ בלבד, ופעולה שהושלמה = NoticeBar/toast. טקסט משנה מותר רק כשהוא מוסיף עובדה שלא נראית (חריג, תוצאה, מגבלה, מקור הנתון). **סוכני AI חייבים לבדוק כל שורת מלל שכתבו מול הכלל הזה לפני שהם מסיימים** — ברירת מחדל: למחוק. **דוגמה 2 — אשף הזמנה חדשה:** "קודם בוחרים לקוח" חוזר בציר הזמן תחת כל אחד מארבעת השלבים הנעולים, שוב בפאנל הסיכום ושוב על הכפתור. כלל: סיבת נעילה **לא** כותבים כטקסט קבוע על שלב נעול — שלב נעול מציג רק אייקון מנעול ושם השלב, והסיבה ב-tooltip; המשך/הכפתור המושבת מסביר פעם אחת לכל היותר, במקום אחד. שלב ראשון תמיד "לקוח", ולכן אין צורך להזכיר זאת. ציר זמן/סטפר מציג שם שלב ומצב (נוכחי / הושלם / נעול) בלבד, בלי שורת מלל נוספת לשלב. **דוגמה 3 — דף הבית:** שורת חיפוש מלווה במלל "מחפש בבת אחת בלקוחות, בהזמנות ובפריטים שהושכרו" — הסבר למה שהחיפוש עושה, שהמשתמש מבין מעצם השימוש. כלל: שדה חיפוש/פעולה מובנת = **בלי משפט הסבר קבוע**; מה שהחיפוש מכסה מופיע לכל היותר כ-placeholder קצר ("חיפוש") או ב-tooltip/ⓘ, ורק אם יש בו חריג ("לא מחפש בהזמנות מחוקות"). תיאור של "מה הפיצ'ר הזה עושה" איננו מידע חדש. כל מחרוזת במילון `strings/` נושאת שדה `why` (מה המידע החדש שהיא נותנת), ומחרוזת בלי `why` נדחית בסקירה. |
| C-1.21 | **יישור קווים — מימין ומלמעלה, בכל מקום שרלוונטי — נוסף 2026-09-25 לבקשת הבעלים** | **בכל כרטיס הבאנר/הרצועה בשמאל מיושר בקצה העליון עם הפרטים שמימין (תחילת אזור התוכן), ולא עם שורת הטאבים.** כלל כללי: (1) כל הרכיבים באותה שורה/עמודה מתחילים באותו קו עליון; (2) כל הרכיבים בעמודה מתחילים באותו קו ימני (RTL) — תוויות, שדות, כותרות ותוכן; (3) אין הזזות של פיקסלים בין אזורים סמוכים; (4) תוכן דביק (`sticky`) נשאר מיושר לקו העליון של אזור התוכן. מימוש: פריסה משותפת (grid/flex) אחת עם קו התחלה משותף, לא `margin-top` מקומי לפיצוי. **אימות מכני**: סקריפט ה-parity משווה `getBoundingClientRect().top` של הרצועה מול אלמנט התוכן הראשון (ו-`right` של עמודות מוערמות) ב-1440 וב-1024 עם סטייה מותרת 0 (עד 1px עיגול). סוכן שסיים עמוד חייב להריץ בדיקת יישור ולהציג את המספרים. |
| C-1.22 | **תאריך עברי בלבד (או לפי הגדרה) — נוסף 2026-09-25 לבקשת הבעלים** | **בכל מקום בממשק התאריך מוצג בעברי בלבד** (למשל "י"ד בסיוון תשפ"ו"). תאריך לועזי מוצג **רק** כשההגדרה הארגונית מאפשרת זאת — קיימת כבר הגדרה `hide_gregorian_calendar` ("אפשר תאריך לועזי ביומן" / הסתרת תאריכים לועזיים); ברירת מחדל: עברי בלבד, והלועזי משני וקטן, לא במקום העברי. אסור תאריך מספרי גולמי (`14.10.2026`, `2026-10-14`) כתאריך הראשי ואסור פורמט תאריך שנכתב ידנית בעמוד: **פונקציית עיצוב אחת בספרייה** (על בסיס `lib/hebrewDate.js`, `getHebrewDateString`, ו-`getIsraelDayRange` לחישובי יום), עם תמיכה בהגדרה, והמחרוזת לא ב-`strings/`. שעה בפורמט 24 שעות. בהדמיות: מתג "שונות בין ארגונים" מדגים גם את ההגדרה. |
| C-1.23 | **כפתור מתחת לשורה מיושר עם הטקסט, לא עם אייקון הצד — נוסף 2026-09-25 לבקשת הבעלים** | בכל שורה/פריט שיש בו אייקון או אווטאר בצד (רשימות, היסטוריה, פריטי הזמנה, תשלומים, התראות, שורות בכרטיס) וכפתור/קישור פעולה מתחת לטקסט: **הקצה הימני של הכפתור מתחיל בדיוק בקו הימני של הטקסט**, לא בקו של האייקון. מימוש: פריסת שורה יחידה (`grid` עם עמודת אייקון + עמודת תוכן) כך שהכפתור נמצא **בתוך עמודת התוכן**, אחרי הטקסט; אסור למקם אותו בשורה נפרדת רחבה מתחת לאייקון או לפצות ב-`margin`/`padding` מקומי. כפתורי פעולה בשורה מתחת לטקסט מתחילים באותו קו ימני בכל השורות (ללא הזחות שונות). אימות מכני: בסקריפט ה-parity `getBoundingClientRect().right` של הכפתור שווה ל-`right` של בלוק הטקסט באותה שורה (סטייה 0–1px) ב-1440, 1024 ו-390; ובפריסת הטלפון גם. |
| C-1.24 | **עמדת לקוח (Kiosk) — שפה ייעודית ללקוחות — נוסף 2026-09-25 לבקשת הבעלים** (משוב: "מסך עמדת לקוח בכלל לא הכיוון — צריך עיצוב מותאם ללקוח, על בסיס העיצוב הקיים אבל ממש לא אותו הדבר") | עמדת הלקוח **אינה מסך צוות במסך גדול.** היא נבנית כשפת עיצוב **נפרדת** שיורשת רק את יסודות המותג (פלטת navy/תכלת/זהב, משפחת הגופן, הטוקנים, האייקונים, ההנפשות והכללים הכלליים: RTL, ניגודיות, תאריך עברי, ריסון תגיות, מלל מיותר), **ולא** את מעטפת הצוות: אין סרגל אתר עליון, אין תפריטי ניהול, אין רצועת שינויים/היסטוריה, אין טבלאות, אין טאבים, אין מונחי צוות. עקרונות: (1) **קהל**: לקוחה/כלה שמסתכלת בשמלות — לא מיומנת בטכנולוגיה; (2) **תמונה תחילה**: שמלות במרכז, קטלוג ויזואלי גדול; (3) **בחירה אחת בכל מסך**, עד 3 פעולות גלויות, כפתורי מגע גדולים (מינימום 64px) וגופן גדול הקריא ממרחק; (4) **חם ומזמין**: טון מלל אישי וקצר, רקע רך, הרבה מרחב, פינות עגולות יותר, תנועה עדינה; (5) **מסך פתיחה (attract)** עם שם הגמ"ח מ-config ואזהרת חוסר פעילות; (6) **אין שום מידע/פעולה פנימיים** (מחירי עלות, לקוחות אחרים, הרשאות); (7) שפת המסך והמותג מ-config לכל גמח; (8) שעון נוכחות והעובדים הם **מסך צוות מגע נפרד** — לא מעורבב עם עמדת הלקוח. ההדמיה של עמדת הלקוח מוגשת כארכיטיפ נפרד (`archetype-kiosk`) ולא כחלק מ-`archetype-touch`. |
| C-1.25 | **איזור תחתית (Footer) בכל דף אתר — נוסף 2026-09-25 לבקשת הבעלים** | בסוף הגלילה של **כל דף באתר הצוות** יש איזור תחתית קבוע ומשותף (רכיב אחד בספרייה, בלי גרסה מקומית לדף), כמקובל באתרים: (1) **קישורים רלוונטיים** מקובצים בעמודות קצרות (למשל: ניווט מהיר לאזורי האתר העיקריים לפי הרשאות המשתמש, עזרה/מדריך, דיווח תקלה, פרופיל והגדרות תצוגה); (2) שורת מידע קטנה: שם הגמ"ח ולוגו (מ-config), גרסת המערכת, ותאריך עברי; (3) קישורים לפי C-1.18 (בלי כחול/קו, פס זהב במעבר), בלי תגיות, מלל מינימלי לפי C-1.20, מיושר לפי C-1.21/C-1.17, RTL. **מיקום**: בסוף התוכן, לא צף ולא דביק; בדף קצר נדבק לתחתית החלון (פריסת עמודה `min-height:100dvh`, התוכן `flex:1`); רקע וגובה אחידים, בלי לחפוף את תוכן הדף, את הודעות ה-toast (שמאל-תחתון) או את הגיליון התחתון בטלפון (מכבד safe-area). בטלפון: עמודות מוערמות, יעדי מגע ≥44px. קישורים מסוננים לפי הרשאות (לא מציג עמוד שהמשתמש אינו רשאי לו) ולפי הגדרות הגמ"ח. **חריגים**: מסך כניסה, עמדת לקוח (Kiosk), שעון נוכחות, דפי הדפסה וחלונות צפות — בלי תחתית (או תחתית מינימלית ייעודית לעמדת הלקוח). |
| C-1.26 | **כפתור אחרי תווית = שורה מונפשת, לא כפתור — נוסף 2026-09-25 לבקשת הבעלים** (דוגמה: "חיפושים אחרונים: כהן, 48131, אלמוג 38") | בכל מקום שבו בתוכן יש כפתור/קישור שבא אחרי תווית או טקסט (חיפושים אחרונים, "עוד N", פעולה מתחת להודעה, ייצוא, פתיחה בשורת טבלה) הוא **אינו נראה ככפתור**: בלי מסגרת ובלי רקע במנוחה, טקסט navy 600, ובמעבר עכבר/מיקוד מקלדת מתמלאת שורה בהירה (`--sky-50`) ונמשך **פס זהב אנכי** (2px, `inset-inline-start:0`, `top/bottom:12px`, `scaleY(0→1)` ב-180ms `var(--ease)`) — בדיוק כשורות החלונות הצפים בסקיצה (`.hf-o`). האלמנט נשאר `<button>`/`<a>` אמיתי עם `:focus-visible` (מסגרת זהב). במגע (`hover:none`) הפס קבוע ב-50%. `prefers-reduced-motion`: בלי מעבר. מימוש יחיד משותף: `.lrow` (בבלוק המעטפת, A11). כפתור עצמאי (פעולה ראשית, אריח קיצור דרך בלי תווית לפניו) נשאר כפתור לפי C-1.29. |
| C-1.27 | **כותרת כרטיס = פס navy כהה + אייקון navy על עיגול זהב עשיר — נוסף 2026-09-25 לבקשת הבעלים** | בראש **כל** כרטיס (`.card > .card-h`) פס כותרת navy (`--navy`) לרוחב הכרטיס עד לשוליים, טקסט הכותרת בטוקן הבהיר (`--sky-50`), ואייקון הכותרת בצבע navy כהה בתוך עיגול/צ׳יפ ברקע **זהב עשיר** (`--gold-rich-c` + שכבת ברק `--gold-rich`; מוגדר פעם אחת כטוקן ובשימוש חוזר, בלי צבע חדש — הערכים מהסקיצה). כל וריאנט צבע של `.ico` (rose/gold/teal…) מתאחד לאותו מראה. מימוש בבלוק המשותף (A9), לא override לפי עמוד. |
| C-1.28 | **כל שינוי צבע רקע באלמנט אינטראקטיבי מונפש — נוסף 2026-09-25 לבקשת הבעלים** | ריחוף / לחיצה / מיקוד / בחירה / מצב "עומדים עליו" משנים `background-color` **במעבר חלק** (`--bg-dur:180ms`, `var(--ease)` של הסקיצה), על כל כפתור, שורה, טאב, שדה, אריח וקישור — אסור החלפה חדה. שדה חיפוש (`.scan`) והכפתור שלידו הופכים **זהב בהיר** (`--gold-light`) בריחוף / `:focus-within`, במעבר מונפש; הכפתור שהעכבר עליו ממש מעמיק לזהב עשיר. `prefers-reduced-motion`: בלי מעבר. שים לב: `background-image` (גרדיאנט) אינו ניתן להנפשה — אנימציה נעשית על `background-color` (הגרדיאנט רק שכבת ברק קבועה). מימוש: A14. |
| C-1.29 | **כפתורים מלאים בצבע רווי — במשׁורה; הטון הוא navy + זהב + תכלת — נוסף 2026-09-25 לבקשת הבעלים** (משוב: "הכפתורים הוורודים בכל מקום") | טון העמוד ורוב הכפתורים והמשטחים = **navy, זהב ותכלת**. מילוי סלמון/אפרסק (`--pbtn`) הוא **מבטא קטן** בלבד — לפעולה שבאמת צריכה הדגשה רוויה (`.btn.danger`, `.btn-pink` מפורש) — אינו הצבע הדומיננטי בשום אזור מסך. כל שאר הכפתורים בטיפול הזהב: גוף זהב בהיר (`--gold-light`) עם מסגרת זהב, שמעמיק לזהב עשיר בריחוף; פעולה ראשית אחת לאזור = זהב עשיר. **חל איסור** על סלמון בפריטים שאינם פעולה ראשית/הרסנית: צ׳יפים של חיפושים אחרונים, אריחי קיצורי דרך, מדיניות פרטיות, ניווט (אלה שורות לפי C-1.26 או כפתורי זהב). טקסט על זהב: AA (navy/שחור). |
| C-1.30 | **עובי מסגרת כרטיס — טוקן יחיד — נוסף 2026-09-25 לבקשת הבעלים** | מסגרת כל כרטיס עבה וברורה: `border: var(--card-bw) solid var(--card-line)` כש-`--card-bw:2px` ו-`--card-line:var(--navy)`. **אסור** עובי/צבע מסגרת קשיחים לפי כרטיס או עמוד; שינוי עובי = שינוי טוקן אחד. מימוש: A8. |
...
| S20 | יישור קווים (C-1.21): רצועה שמאלית מיושרת לקו העליון של אזור התוכן (לא לטאבים); עמודות מיושרות מימין | בדיקת bounding-box ב-1440/1024 בסקריפט ה-parity |
| S21 | תאריך עברי בלבד (C-1.22): אין תאריך לועזי/מספרי גולמי כתאריך ראשי; לועזי רק לפי `hide_gregorian_calendar` | חיפוש regex של `\d{1,2}[./]\d{1,2}[./]\d{2,4}` בעמוד + סקירה |
| S22 | יישור כפתור מתחת לשורה (C-1.23): הכפתור מתחיל בקו הימני של הטקסט, לא של האייקון | בדיקת bounding-box לכל שורת רשימה עם כפתור |
| S23 | עמדת לקוח (C-1.24): שפה ייעודית ללקוח — אין מעטפת צוות (סרגל/טאבים/טבלאות/מונחי צוות), יעדי מגע ≥64px, בחירה אחת למסך | סקירה + מדידת גדלי מגע ב-390 וב-1024 |
| S24 | תחתית (C-1.25): בכל דף צוות רכיב תחתית משותף בסוף הגלילה; נדבק לתחתית בדף קצר; לא חופף ל-toast/גיליון | בדיקת bounding-box בדף קצר ובדף ארוך ב-390/1024/1440 |
| S25 | כפתור אחרי תווית (C-1.26): אין `<button>` בעל מסגרת/רקע במנוחה שבא אחרי טקסט תווית; שורה מונפשת עם פס זהב אמיתי | `getComputedStyle` במנוחה (`border-width:0`, רקע שקוף), ריחוף: `::before` `transform` ≠ scaleY(0) ו-`transition-duration>0`, מיקוד: `:focus-visible` |
| S26 | כותרת כרטיס (C-1.27): לכל `.card>.card-h` רקע `--navy`, טקסט `--sky-50`, אייקון navy על רקע זהב עשיר | `getComputedStyle` של `.card-h`, `h2`, `.ico` בכל כרטיס בכל מצב |
| S27 | אנימציית רקע (C-1.28): כל אלמנט אינטראקטיבי ששינוי ריחוף משנה לו `background-color` מגדיר `transition-duration>0` על `background-color`/`all`; `.scan` והכפתור שלידו זהב בהיר בריחוף/מיקוד | סוויפ: ריחוף על כל button/a/input, השוואת רקע במנוחה/ריחוף + `transition-*` |
| S28 | מילוי רווי מינימלי (C-1.29): מילויי סלמון/אפרסק הם מיעוט ברור (מתחת ~20%) מהפקדים הממולאים לכל אזור מסך ואינם הטון הדומיננטי; לא בצ׳יפים/קיצורי דרך/פרטיות | סריקת `background-color` בטווח גוון 5°–30° לכל אלמנט בכל מצב + ספירת פקדים ממולאים ויחס |
| S29 | עובי מסגרת כרטיס (C-1.30): `border-top-width` של כל `.card` = ערך הטוקן `--card-bw` (2px), צבע = `--card-line` | `getComputedStyle` לכל כרטיס + grep אין `border:` קשיח על `.card` בעמוד |
