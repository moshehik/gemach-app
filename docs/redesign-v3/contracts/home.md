...
## יב. עריכת אב-טיפוס 2026-09-25 (בקשת הבעלים)
- **רוחב (עמודי, לא חוקה):** כל כרטיס בדף הבית = 65% מהרוחב הקודם, מתחיל בימין (RTL), בכל הרוחבים כולל 390 (233px); צפיפות פנימית מוקטנת (padding, גופן, אייקונים, אריחי קיצורים; יעדי מגע >=44px). הסרגל העליון נשאר ברוחב מלא.
- **C-1.20:** נמחקו משפט ההסבר מתחת לשורת החיפוש, "מחוברת: <שם>", "תוצאות עבור ...", והערת BDQ-D4 (הפכה ל-tooltip ⓘ בכרטיס הזמנות). placeholder = "חיפוש"/"שאלה".
- **C-1.26:** חיפושים אחרונים, "עוד N", פעולות תחת הודעת ה-AI, שורות תוצאה = `.lrow`. מדיניות פרטיות עברה לתחתית המשותפת (C-1.25), אותו handler (`data-act=privacy`).
- **C-1.19 טבלת פקדים:** מצב חיפוש (2, תווית), שדה + שליחה, ניקוי (אייקון: aria-label+tip), חיפושים אחרונים (עד 5), 5 אריחי קיצור (tip), תחתית (ניווט/עזרה/חשבון).
