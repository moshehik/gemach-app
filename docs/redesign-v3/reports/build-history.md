...
## תיקוני אימות (verify-history.md) — עדכון
- **B1:** שדה "טכני" ששונה באמת לעולם לא נזרק. הוא מוצג בקטע מקופל "שדות טכניים" בפאנל המורחב; UPDATE שכולו טכני נשאר כרשומה ("נתונים טכניים: …"). מושמטים רק `id/legacyId/createdAt/updatedAt`.
- **B2:** replay והסקת "לפני" רצים **רק** על `action==='UPDATE'`. אירועים (EMAIL_SENT, ADD_PAYMENT, CANCEL_*…) ו-CREATE מועתקים כמו שהם ואינם משנים מצב; השוואה עמוקה (JSON) לאובייקטים/מערכים.
- **B3:** נדחה רק שוויון **מתועד** (`from===to` בשורה עצמה). כשה"לפני" הוסק מהמצב הידוע והוא שווה ל"אחרי" — נוצרת רשומה מסומנת "משוער" (`estimated`), עם מתג "הסתר שמירות ללא שינוי מזוהה" (כבוי כברירת מחדל). ערכי "לפני" מוסקים מסומנים "(משוער)" בפירוט. **שינוי מול סיכון 1 לעיל:** רעש שנוצר משמירת הזמנה מלאה מוצג כעת כברירת מחדל בשורה משוערת, ולא נדחה.
- קיפול ביטול הזמנה: לפי orderId (ילד נבלע רק אם `orderId` בשורה שלו זהה, או כש-`ctx.singleOrder` בפיד הזמנה אחת); סכום החיובים שבוטלו מוצג.
- ניסוח Refund UPDATE: "עודכן זיכוי", בלי צ'יפ סכום ("נוצר" רק ב-CREATE).
- חיתוך 500: `mergeRows` מחזיר `totalAll`; כשגדול מהנטען נראית שורת "מוצגות X מתוך Y" עם "חיפוש בכל ההיסטוריה" (סינון שרת חל על כל 5 המקורות).
- שעה: `hourCycle:'h23'` (00:xx).
- הבדיקה `history-adapter-selftest.mjs` כוללת רגרסיות ל-B1/B2/B3, קיפול, Refund ושעה — עוברת. eslint נקי.
- **פתוח:** פיצול המילונים לקובץ נפרד (המלצת המאמת) לא בוצע; טעינת עוד במסלול מאוחד — רק הודעה + חיפוש שרת.
