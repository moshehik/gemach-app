...
## תיקוני אימות (סבב 2)
- חזרה-לסיכום נוקה בכל ניווט דרך הסטפר וכפתור "חזרה"; חל רק על הזרימה שהתחילה ב"עריכה".
- עריכת לקוח מהסיכום עוברת דרך שלב התאריכים (שדות משלוח) ורק אחרי ההמשך ממנו חוזרת לסיכום.
- Esc בחלונית אישור מנהל (PopupProvider) לא סוגר את חלונית הלקוח הכפול שמתחתיה (דגל `authBusyRef`).
