/**
 * history-adapter-skeleton.js  --  שלד, לא מחובר לשום מקום (R23).
 *
 * שכבת קריאה/תצוגה בלבד: מקבלת שורות AuditLog גולמיות (כפי שמחזיר /api/audit אחרי
 * attachEmployeeNames) ומחזירה "רשומות פיד" במבנה שהסקיצה (order-card-sketch-B.html) מצפה לו:
 *   {id, ts, cat, icon, text, sub, who, amt, kind, det:[[label,value]], ...}
 * אין כאן שום כתיבה. אין ולא תהיה כתיבת AuditLog ידנית - התוסף ב-app/lib/prisma.js כותב הכול.
 *
 * מיקום מוצע בהטמעה: lib/historyAdapter.js (+ lib/historyDictionary.js לטבלאות).
 * מסמך מלא: docs/redesign-v3/HISTORY-DESIGN.md
 */
...
export function replay(parsed) {
  ...
  for (const { row, p } of asc) {
    ...
    for (const [f, c] of Object.entries(p.fields)) {
      ...
      if (!hasFrom && p.shape === 'snapshot' && (to === false || empty(to))) { st[f] = to; continue; } // ברירות מחדל ריקות ביצירה
...
export function toFeedEntry(row, p, ctx = {}, stateLookup = () => ({})) {
  ...
  if (A === 'UPDATE' && visible.length === 0 && p.shape !== 'unparsable') return null;
  ...
  return {
    ...
    badge: act?.badge || ent.label, tone: act?.tone || 'neutral',
...
function genericSentence(E, A, visible, F, ctx, row) {
  ...
  if (visible.length === 1) return `${FIELDS[visible[0]]?.[0] || visible[0]} עודכן${E === 'Customer' ? 'ה' : ''}`;
...
function buildDetails(row, p, visible) {
  ...
  const rows = visible.map((k) => {
    const c = p.fields[k]; const lbl = FIELDS[k]?.[0] || k; // שדה לא מוכר: מפתח גולמי (fallback, נרשם ליומן חסרים)
...
export const searchHay = (e) => [e.text, e.sub, e.who, e.badge, e.ts.slice(11), ...(e.det || []).flat(), e.raw || ''].join(' ').toLowerCase();
 ...
 */
export async function loadOrderHistoryRows(order, fetchJson = (u) => fetch(u).then((r) => (r.ok ? r.json() : { logs: [] }))) {
  ...
  const [o, it, pay, ob, rf] = await Promise.all([
    fetchJson(`/api/audit?entityType=Order&entityId=${order.orderId}&limit=500`), // כבר מטפל ב-uuid + orderId
  ...
  const seen = new Set();
  return [o, it, pay, ob, rf].flatMap((r) => r.logs || []).filter((l) => !seen.has(l.id) && seen.add(l.id));
}
