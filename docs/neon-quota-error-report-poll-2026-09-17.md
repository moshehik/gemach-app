...
| **נשאר בארגון האישי** (`org-young-fire-22908884`) | — | `fancy-hill-59651059` ("bagrut-materials"), `divine-credit-39212163` ("print-center"), `dry-sky-36955442` ("print-center-NOW") | ⏳ **לא נבדקו ברמת שורות, לא נמחקו** — הבעלים ביקש לבדוק לפני מחיקה. נסיבתית: כולן נוצרו 25-26/8, אותו יום כמו שאר הכפילויות שכבר אומתו כישנות/קפואות — סביר שאותו דפוס, אבל לא אומת ישירות כמו `quiet-forest`/`morning-morning`. |

**איך לסיים את הניקוי**: לבדוק כל אחד מ-3 הפרויקטים שנשארו (שורות בטבלאות המרכזיות של כל אתר, מול הפרויקט התואם בארגון הייעודי) לפני מחיקה — לא למחוק על סמך נסיבתי בלבד. אחרי שהשלושה האלה ייבדקו וינוקו, הארגון האישי יהיה ריק לגמרי וניתן "לסגור" אותו (נאון לא תומך במחיקת ארגון ריקה ב-API ללא Referer, כנראה גם זה דרך הקונסולה).
