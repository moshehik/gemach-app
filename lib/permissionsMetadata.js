...
export const PERMISSION_CATALOG = [
  ...
  },
  {
    ...
    type: 'boolean',
    enforced: false,
    note: 'נשלט כרגע לפי app/refunds/layout.js — הנהלה ראשית/מתכנת (roleId 0/2), ובנוסף מנהל סניף (roleId 1) כשהגדרת המערכת restrict_refunds_to_head_management כבויה. (עד 2026-09-20 הנהלה ראשית נחסמה בטעות כשההגדרה כבויה.) ה-API של הזיכויים עדיין פתוח לכל עובד מחובר — ר\' CLAUDE.md.',
  ...
  // ---------------------------------------------------------------------
  {
    ...
    type: 'boolean',
    enforced: true,
    note: 'מיושם בשרת: checkAiAccess() ב-lib/permissions.js נקרא מכל הנתיבים תחת app/api/ai/** (מאז 2026-09-20; קודם רק הסתרת הרכיב ב-app/layout.js — עובד יכול היה לקרוא ל-API ישירות), וכן app/layout.js מסתיר את הרכיב לפי אותו כלל. הנהלה ראשית/מתכנת תמיד מורשים. הרשאה פרטנית לעובד נשארת ב-Employee.showAi הקיים (ר\' כרטיס עובד) — יכולה רק להוסיף גישה, לא לשלול אותה. מתג המערכת hide_ai_features מכבה AI לכולם.',
    userNote: 'קובע מי יכול להשתמש בבינה המלאכותית (החיפוש החכם, הצ\'אט ואשף הדוחות). הנהלה ראשית ומתכנת תמיד מורשים. אפשר להוסיף גישה לעובד מסוים גם מכרטיס העובד, אבל אי אפשר לשלול ממנו גישה שמגיעה לו לפי המחלקה.',
    defaultForRoleId: (roleId) => HEAD_MANAGEMENT.includes(roleId),
