...
export const PERMISSION_CATALOG = [
  ...
  // ---- "מי רשאי לאשר" — הרשאות אישור בסיסמה שעד 2026-09-22 נקבעו בהגדרות המערכת או בקוד קשיח ----
  // approver:true = הפעולה מבקשת סיסמה של מאשר (חלון "אימות הרשאה", ר' PopupProvider.js), והפריט הזה
  // קובע מי נחשב מאשר תקף: אותה החלטה בבורר העובדים בלקוח (approvals ב-GET /api/employees), ב-verify-pin
  // (requiredLevel = מפתח הפריט) ובשרת עצמו. approverLabel = הכיתוב בחלון האישור ("בחר <approverLabel>").
  // מתגי ההפעלה עצמם (האם בכלל נדרש אישור) נשארים הגדרות מערכת גלובליות - כאן רק "מי".
  {
    ...
    settingKeys: ['allow_shift_lead_reserve_rental'],
    note: 'נאכף ב-app/api/rentals/scan/route.js (חסימת רזרבה טהורה בלבד - "מחסן" נשאר תמיד למנהל סניף/מתכנת) ובבורר המאשר ב-components/orders/RentalReturnModal.js. ברירת המחדל תלויה בהגדרת המערכת allow_shift_lead_reserve_rental: מופעלת = כל עובד פעיל יכול לאשר; כבויה = מנהל סניף בלבד (הנהלה ראשית/מתכנת תמיד). שורת הרשאה גוברת. ההתראה הפנימית למנהלים נשלחת כשהמאשר אינו מנהל סניף/הנהלה ראשית/מתכנת. הגדרה זו הייתה עד 2026-09-22 מתג בלבד בהגדרות המערכת (טאב "מלאי").',
    userNote: 'קובע מי יכול לאשר בסיסמה השכרה של שמלה שמסומנת "רזרבה". אם לא הוגדרה שורה, נקבע לפי הגדרת המערכת "אפשר לאחראית משמרת לאשר השכרת רזרבה" (מופעלת = כל עובד, כבויה = מנהל סניף). שמלה שמסומנת "מחסן" נשארת תמיד לאישור מנהל סניף או מתכנת. הנהלה ראשית ומתכנת תמיד מורשים.',
    defaultForRoleId: (roleId, s) => (s && s.allow_shift_lead_reserve_rental === 'true' ? true : MANAGER_TIER.includes(roleId)),
  },
  {
    ...
    settingKeys: ['PAYMENT_APPROVAL_LEVEL'],
    note: 'נאכף ב-app/orders/new/page.js (הבורר והאימות ב-/api/auth/verify-pin עם requiredLevel="feature:payment_exit_approval"). החלונית נפתחת רק כשהגדרת המערכת PAYMENT_APPROVAL_LEVEL אינה "כולם". ברירת המחדל נגזרת מאותה הגדרה: "עובד" = כל עובד פעיל, "מנהל" = מנהל סניף/מתכנת, "מנהל סניף ומעלה" = גם הנהלה ראשית. שורת הרשאה גוברת. (אישור של הזמנה קיימת נשלט בנפרד ב-feature:debt_approval.) הגדרה זו הייתה עד 2026-09-22 רק הגדרת מערכת.',
    userNote: 'קובע מי יכול לאשר בסיסמה סיום של הזמנה חדשה בלי תשלום מלא. החלונית מופיעה רק כשבהגדרות המערכת "רמת אישור ליציאה מהזמנה בלי תשלום מלא" אינה "כולם", ואם לא הוגדרה שורה - היא קובעת גם מי מורשה. הזמנה שכבר קיימת נשלטת בהרשאה "אישור הזמנות ללא תשלום מלא". הנהלה ראשית ומתכנת תמיד מורשים.',
    defaultForRoleId: (roleId, s) => {
      const level = (s && s.PAYMENT_APPROVAL_LEVEL) || 'כולם';
      if (level === 'מנהל') return MANAGER_TIER.includes(roleId);
      if (level === 'מנהל סניף ומעלה') return [0, 1, 2].includes(roleId);
      return true; // 'כולם' / 'עובד' - כל עובד פעיל מזהה את עצמו בסיסמה
    },
  },
  {
    ...
    defaultForRoleId: (roleId) => MANAGER_TIER.includes(roleId),
    note: 'נאכף בשרת ב-PUT /api/orders/[id] וב-POST /api/orders/[id]/items דרך verifyManagerPin(..., "feature:item_change_approval") ב-lib/managerAuth.js, ובבורר המאשר ב-app/orders/[id]/page.js וב-components/orders/modern/ModernItemsManager.js. האישור נדרש רק כשהגדרת המערכת require_manager_code_for_item_changes מופעלת. ברירת המחדל = מנהל סניף/מתכנת כמו קודם; הנהלה ראשית (roleId 0) מורשית תמיד ולכן נוספה כמאשרת תקפה (קודם verifyManagerPin לא קיבל אותה).',
    userNote: 'קובע מי יכול לאשר בסיסמה ביטול פריט או הוספת פריט חדש להזמנה שכבר נשמרה. האישור נדרש רק כשבהגדרות המערכת מופעל "דרוש קוד מנהל לביטול/הוספת פריט". ברירת המחדל: מנהל סניף. הנהלה ראשית ומתכנת תמיד מורשים.',
  },
  {
    ...
    defaultForRoleId: (roleId) => MANAGER_TIER.includes(roleId),
    note: 'נאכף ב-lib/rentalBarcodeGuard.js (verifyManagerOverride) ובבורר המאשר ב-components/orders/RentalReturnModal.js וב-components/orders/modern/rentalToggle.js. פועל רק כשהגדרת המערכת enforce_rental_barcode_match מופעלת. ברירת המחדל = מנהל סניף/מתכנת כמו קודם; הנהלה ראשית מורשית תמיד.',
    userNote: 'קובע מי יכול לאשר בסיסמה השכרה של שמלה שהדגם או המידה שלה לא תואמים למה שהוזמן. הבדיקה פעילה רק כשבהגדרות המערכת מופעל "חסום השכרה של ברקוד שלא תואם". ברירת המחדל: מנהל סניף. הנהלה ראשית ומתכנת תמיד מורשים.',
  },
  {
