...
export const PERMISSION_CATALOG = [
  ...
  approverItem({
    key: 'feature:early_return_approval',
    label: 'אישור החזרה לפני מועד האירוע',
    description: 'לאשר בסיסמה סימון פריט כהוחזר בהזמנה שתאריך האירוע שלה עדיין לא הגיע.',
    approverLabel: 'מאשר החזרה מוקדמת',
    note: 'נאכף ב-lib/earlyReturnGuard.js (verifyManagerOverride), דרך POST /api/returns/scan (app/rentals/page.js, components/orders/RentalReturnModal.js) ו-POST /api/rentals/toggle (components/orders/modern/ModernItemsManager.js). פועל רק כשהגדרת המערכת require_approval_for_early_return מופעלת. דווח כדיווח תקלה 8efa57cc, 2026-09-22.',
    userNote: 'קובע מי יכול לאשר בסיסמה סימון פריט כהוחזר כשתאריך האירוע של ההזמנה עדיין לא הגיע. הבדיקה פעילה רק כשבהגדרות המערכת מופעל "דרוש אישור מנהל להחזרה לפני מועד האירוע".',
  }),
  approverItem({
  ...
  approverItem({
    key: 'feature:order_date_edit_approval',
    label: 'עריכת תאריך ביצוע ההזמנה',
    description: 'לאשר בסיסמה עריכה של תאריך ביצוע ההזמנה (orderDate) - משפיע על חלון הזיכוי בביטול במנוע התמחור.',
    approverLabel: 'מאשר עריכת תאריך הזמנה',
    note: 'components/orders/modern/ModernGeneralDetails.js ו-components/orders/modern/ModernInfoTab.js (שני requestOrderDateEdit נפרדים, אותו שדה). עד 2026-09-23 קשיח: roleId 2 בלבד (מתכנת). נאכף גם בשרת מ-2026-09-23 ב-PUT /api/orders/[id] (data.orderDate מול existingOrder.orderDate, ר\' verifyManagerPin עם data.orderDateApproverId/orderDateApproverPin באותו route) - קריאת API ישירה כבר לא יכולה לעקוף.',
    userNote: 'קובע מי יכול לאשר בסיסמה עריכה של תאריך ביצוע ההזמנה בכרטיס הזמנה קיים.',
  }),
  approverItem({
    key: 'feature:manual_payment_credit_add',
    label: 'הוספת תשלום/זיכוי ידני מהזמנה',
    description: 'לאשר בסיסמה רישום תשלום ידני (למשל מזומן) או בקשת זיכוי ללקוח, דרך הכפתור המאוחד בטאב "פרטים כלליים".',
    approverLabel: 'מאשר תשלום/זיכוי ידני',
    note: 'נאכף בקליינט ב-app/orders/[id]/page.js (handleOpenManualPaymentCredit, דרך window.customAuthPrompt + POST /api/auth/verify-pin עם requiredLevel="feature:manual_payment_credit_add") לפני קריאה ל-openAdditionalPaymentModal/openRefundModal ב-ModernPaymentsManager.js (חשוף דרך ref). הכפתור המאוחד עצמו (components/orders/modern/ModernGeneralDetails.js) מוצג רק כש-SystemSetting consolidate_manual_payment_credit_ui מופעל - כרגע true רק אצל נווה יעקב; אצל שאר הגמחים (המתג כבוי) שני הכפתורים הנפרדים "תשלום נוסף"/"בקשת זיכוי ללקוח" נשארים בטאב תשלומים בלי שער הרשאה נוסף, כמו קודם.',
    userNote: 'קובע מי יכול לאשר בסיסמה הוספת תשלום ידני או בקשת זיכוי ללקוח, כשמופעלת התצוגה המאוחדת (כרגע: נווה יעקב בלבד). אינו משפיע על גמח שנשאר בתצוגה הרגילה.',
  }),
  approverItem({
  ...
  {
    key: 'feature:ai_financial_data',
    group: 'features',
    label: 'חשיפת נתונים פיננסיים מהבינה המלאכותית',
    description: 'לקבל מהבינה המלאכותית (צ\'אט וסטטיסטיקות) נתונים פיננסיים - הכנסות, שכר, סטטיסטיקות עסקיות כלליות.',
    type: 'boolean',
    enforced: true,
    note: 'מיושם ב-loadEmployeeAccess() ב-lib/ai/aiCommon.js דרך hasPermission() - משותף לצ\'אט הראשי (app/api/ai/route.js) ולעוזר הסטטיסטיקה (app/api/ai/statistics/route.js). עד 2026-09-22 קשיח בשני המקומות בנפרד (roleId===1||roleId===2), עם באג: הנהלה ראשית (roleId 0) לא נחשבה "מנהל" שם בניגוד לכל שאר המערכת. ברירת המחדל שומרת על ההתנהגות הקודמת: מנהל סניף (roleId 1) מורשה, הנהלה ראשית/מתכנת תמיד מורשים (ALWAYS_ALLOWED_ROLE_IDS).',
    userNote: 'קובע אם הבינה המלאכותית רשאית לחשוף למשתמש נתונים פיננסיים (הכנסות, שכר, סטטיסטיקות עסקיות כלליות). הנהלה ראשית, מנהל סניף ומתכנת מורשים כברירת מחדל.',
    defaultForRoleId: (roleId) => roleId === 1 || HEAD_MANAGEMENT.includes(roleId),
  },
  {
