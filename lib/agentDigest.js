...
import { renderAgentDigestEmailHtml } from '@/lib/emailTemplates';
...
export async function runAgentDigest({ slot } = {}) {
  ...
  const html = renderAgentDigestEmailHtml({
    prs: prs.map(pr => ({
      number: pr.number,
      title: pr.title,
      branch: pr.head.ref,
      created: new Date(pr.created_at).toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }),
      excerpt: (pr.body || '').trim().replace(/\r?\n+/g, ' ').slice(0, 220),
      url: pr.html_url,
    })),
  });
  ...
  for (const emp of recipients) {
    const r = await sendSystemEmail({
      to: emp.email,
      subject: emailSubject('agentDigest', { count: prs.length }),
      body: bodyText,
      html,
    });
