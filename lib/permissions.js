...
//     otherwise their department row, an employee override, or Employee.showAi);
...
export async function canApproveDebt(employeeId) {
  ...
  const employee = await prisma.employee.findUnique({
    where: { id: employeeId },
    select: { id: true, roleId: true, isActive: true, showAi: true, canReportErrors: true },
  });
  if (!employee || !employee.isActive) return false;
  return hasPermission(employee, 'feature:debt_approval');
}

// --- page:* items, enforced at the page (layout.js) and in the sidebar ---------------------
// One place decides "may this employee open this page": head management / programmer always;
// otherwise the employee's own override, else their department's explicit value, else the catalog
// default (which, for refunds / dress catalog / monthly board, follows the org's restrict_* setting -
// exactly what those layouts used to check by hand). Both the layouts (canOpenPage) and the sidebar
// (resolvePageAccess in app/layout.js) call this, so a link is shown exactly when the page opens.
async function getExplicitValues(roleId, employeeId) {
  ...
  const [deptRows, overrideRows] = await Promise.all([
    roleId === null || roleId === undefined ? [] : prisma.departmentPermission.findMany({ where: { roleId, key: { startsWith: 'page:' } } }),
    employeeId ? prisma.employeePermissionOverride.findMany({ where: { employeeId, key: { startsWith: 'page:' } } }) : [],
  ]);
  const value = {
