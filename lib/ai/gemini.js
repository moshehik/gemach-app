...
// media: array of { mimeType, data } (base64, inline - images/short clips) and/or
// { mimeType, fileUri } (Gemini Files API reference - large video, see
// lib/ai/geminiFiles.js). Kept as a separate optional param (not just appended to
// the prompt string) so every existing text-only call site stays unchanged.
export async function generateContent(prompt, requestedModel = null, media = []) {
  ...
  const mediaParts = (media || []).map((m) => (
    m.fileUri
      ? { file_data: { file_uri: m.fileUri, mime_type: m.mimeType } }
