...
export async function loadEmployeeAccess(prisma, cookieStore) {
  ...
  if (token && token.value) {
    ...
    if (employee) {
      // feature:ai_financial_data בקטלוג ההרשאות (lib/permissionsMetadata.js) - עד 2026-09-22 היה
      // קשיח roleId===1||roleId===2 כאן ובעותק נפרד ב-app/api/ai/route.js, עם באג: הנהלה ראשית
      // (roleId 0) לא נחשבה "מנהל" בניגוד לכל שאר המערכת.
      isManager = await hasPermission(employee, 'feature:ai_financial_data');
      employeeContext = isManager
