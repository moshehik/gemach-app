...
import { ORDER_REDIRECT_SCREENS } from '@/lib/orderRedirectScreens';
...
export const SETTINGS_HEBREW_NAMES = {
  ...
  mandatory_field_groups: 'קבוצות "אחד מספיק" בשדות חובה',
  ...
  consolidate_manual_payment_credit_ui: 'איחוד תשלום/זיכוי ידני לכפתור אחד',
...
export const SETTINGS_HEBREW_NOTES = {
  ...
  mandatory_fields: 'סמן בתיבת הבחירה (צ\'קבוקס) את השדות מתוך פרטי לקוח שיהיו חובה בעת מילוי הזמנה - חוסמים שמירה עד שימולאו, עם כוכבית אדומה ליד השדה בטופס. הרשימה כוללת רק שדות שבאמת נאכפים בטופס הזמנה חדשה (שם פרטי/משפחה/טלפון ראשי/אימייל/עיר/רחוב/מספר בית); לדרישת "אחד מספיק מבין שני שדות" (למשל טלפון נוסף/אימייל) יש הגדרה נפרדת למטה.',
  mandatory_field_groups: 'קבוצות שדות "לפחות אחד מהם חובה" (למשל טלפון נוסף / אימייל) - בנפרד משדות חובה רגילים (חובה תמיד) שלמעלה. ברירת המחדל (כשריק): טלפון נוסף/אימייל, כמו שהיה קודם.',
  ...
  consolidate_manual_payment_credit_ui: 'כשמופעל: מסתיר את כפתורי "תשלום נוסף" ו"בקשת זיכוי ללקוח" מטאב תשלומים, ומציג במקומם כפתור מאוחד אחד בטאב "פרטים כלליים" - הוספת תשלום/זיכוי משם דורשת קוד מאשר (הרשאת "הוספת תשלום/זיכוי ידני מהזמנה" בדף ההרשאות). כברירת מחדל (כבוי) שום דבר לא משתנה.',
...
export const SETTINGS_ORDER = {
  ...
  'הזמנות': [
    ...
    'mandatory_fields', 'mandatory_field_groups', 'strict_mandatory_fields', 'require_id_for_edit_cancel',
    ...
    'order_new_redirect_screen', 'order_edit_redirect_screen',
  ],
  ...
  'תשלומים': [
    ...
    'ALLOWED_PAYMENT_METHODS', 'PAYMENT_APPROVAL_LEVEL', 'allow_additional_payment_on_order', 'consolidate_manual_payment_credit_ui',
...
export const SETTINGS_BOOLEAN_KEYS = [
  ...
  'allow_additional_payment_on_order', 'hide_taken_orders_from_orders_list', 'consolidate_manual_payment_credit_ui',
...
export const SETTINGS_SELECT_OPTIONS = {
  ...
  order_new_redirect_screen: ORDER_REDIRECT_SCREENS,
  order_edit_redirect_screen: ORDER_REDIRECT_SCREENS,
};

const MANDATORY_FIELDS_KEY = 'mandatory_fields';
const MANDATORY_FIELD_GROUPS_KEY = 'mandatory_field_groups';
...
export function classifySettingField(key, rawType, rawValue) {
  ...
  if (key === MANDATORY_FIELD_GROUPS_KEY) return 'fieldGroups';
