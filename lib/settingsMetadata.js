...
export const SETTINGS_BOOLEAN_KEYS = [
  ...
  'enable_batch_print_prep', 'same_model_swap_no_fee'
];
...
// מפתחות "hide_*" מוצגים למשתמש הפוך מהערך הגולמי ב-DB - ר' isHideSetting/uiValue
// ב-SettingsClient.js. לדוגמה hide_custom_spacing='true' (=מוסתר) מוצג כ"כבוי",
// כי הכיתוב במסך הוא הניסוח החיובי "אפשר ציפוף ימים מיוחד" ולא "הסתר...". בלי
// ההיפוך הזה גם התשובה של ה-AI וגם המתג בפאנל המהיר מראים את המצב ההפוך מהאמיתי.
...
export function toDisplayValue(key, value) {
  if (key.startsWith('hide_') && (value === 'true' || value === 'false')) {
