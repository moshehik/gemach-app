...
export function calculateOrderStatus(order) {
  ...
  // An order the new-order screen saved on its own and never finished. The stored status is
  // the only thing that tells it apart - by items and dates it looks like any other order,
  // so deriving the status from those alone showed an abandoned cart as 'בקרוב'.
  ...
  if (validItems.length > 0) {
    ...
    const allTaken = validItems.every(item => item.isTaken || item.barcode !== null);
    const someTaken = validItems.some(item => item.isTaken || item.barcode !== null);
