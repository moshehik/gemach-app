...
/**
 * מייל "נתונים למשלוחן" (§D, docs/deliveries-feature-plan-2026-09-16.md) - טבלת
 * שם/כתובת/2 טלפונים לכל קבוצת כיוון+תאריך אירוע, עם אותה כותרת שמופיעה בהדפסה
 * (lib/deliveryCourier.js buildCourierGroupTitle). משתמש ב-.files-table הקיים
 * (טבלת נתונים גנרית, לא meta-table הדו-טורי) כי כאן צריך 4 עמודות אמיתיות.
 *
 * @param {{groups: Array<{title:string, rows: Array}>, gmachName?: string}} params
 */
export function renderCourierDeliveryEmailHtml({ groups, gmachName = 'גמ"ח שמלות' }) {
  const sectionsHtml = (groups || []).map(group => {
    const rowsHtml = (group.rows || []).map(r => `
      <tr>
        <td style="font-weight:600;color:#333;">${escapeHtml(r.customerName)}</td>
        <td>${escapeHtml(r.address || '-')}</td>
        <td style="white-space:nowrap;" dir="ltr">${escapeHtml(r.customerPhone || '-')}</td>
        <td style="white-space:nowrap;" dir="ltr">${escapeHtml(r.customerPhone2 || '-')}</td>
      </tr>
    `).join('');
    return `
      <div class="files-section-title">${escapeHtml(group.title)}</div>
  ...
  const bodyHtml = `
    <div class="greeting">נתוני משלוחים</div>
    ${sectionsHtml || '<div class="body-text">אין משלוחים בטווח/כיוון שנבחר.</div>'}
  `;

  return renderShell({ gmachName, subtitle: 'נתוני משלוחים למשלוחן', bodyHtml });
}
