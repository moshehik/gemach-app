...
/** התבקש מענה אנושי בדיווח תקלה - אדום. */
export function renderHumanRequestedEmailHtml({ reporterName = '', pageTitle = '', description = '', gmachName = 'גמ"ח שמלות' }) {
  const bodyHtml = `
    ${kvCard([{ label: 'מדווח/ת', value: reporterName, strong: true }, { label: 'חלון/דף', value: pageTitle }], 'red')}
    ${sectionTitle('תיאור התקלה', 'red')}
