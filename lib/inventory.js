...
export async function validateOrderItemsAvailability(items, eventDate, isAbroad, fromDate, toDate, orderId = null, customSpacing = null) {
  ...
  // Each modelId's availability check is independent (loadInventoryContext builds a fresh
  // context per call, no shared/mutated state across calls), so run them concurrently instead
  // of one dress-model at a time.
