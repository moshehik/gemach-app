  ...
  for (const tDate of validDates) {
    ...
    for (const booking of bookings) {
      ...
      if (customStart && customEnd) {
        // Custom booking overlaps if its custom range intersects with target bounds
        if (Math.max(boundMin.getTime(), customStart.getTime()) <= Math.min(boundMax.getTime(), customEnd.getTime())) {
          overlaps = true;
        }
      } else if (bookingEventDate) {
        // Standard booking overlaps if its buffered range intersects with target bounds
        ...
        if (Math.max(boundMin.getTime(), bMin.getTime()) <= Math.min(boundMax.getTime(), bMax.getTime())) {
          overlaps = true;
        }
      }
...
export async function getBulkAvailableInventory(targetDate) {
  ...
  for (const booking of bookings) {
    ...
    let overlaps = false;
    if (customStart && customEnd) {
      const boundMin = addDaysSkippingWeekends(minDate, -bufferDays, skipWeekends);
      const boundMax = addDaysSkippingWeekends(minDate, bufferDays, skipWeekends);
      if (Math.max(boundMin.getTime(), customStart.getTime()) <= Math.min(boundMax.getTime(), customEnd.getTime())) overlaps = true;
    } else if (bookingEventDate) {
      const boundMin = addDaysSkippingWeekends(minDate, -bufferDays, skipWeekends);
      const boundMax = addDaysSkippingWeekends(minDate, bufferDays, skipWeekends);
      ...
      if (Math.max(boundMin.getTime(), bMin.getTime()) <= Math.min(boundMax.getTime(), bMax.getTime())) overlaps = true;
    }
