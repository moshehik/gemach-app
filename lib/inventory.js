...
export async function getBulkAvailableInventory(targetDate, modelIds = null) {
  ...
  const bookings = await prisma.orderItem.findMany({
    where: {
      ...
      order: {
        // הוסר סינון status:{notIn:['מבוטל','מחוק']} - Order.status כמעט אף פעם לא מאוכלס
        // בפועל (הסטטוס האמיתי מחושב דינמית ב-lib/orderStatus.js), ו-SQL "NOT IN" עם NULL
        // מחזיר unknown/false - כך שכל הזמנה עם status ריק נפלה בשקט מהתוצאה. באירוע אמיתי
        // (נווה יעקב) זה גרם לכמעט כל ההזמנות לא להיספר כתופסות מלאי במסך עמדת הלקוח.
        // isDeleted:false כבר מסנן הזמנות מבוטלות/מחוקות, כמו ב-buildBookingsWhere למעלה.
        isDeleted: false,
        OR: [
...
export async function checkMissingDressForItem({ dressModelId, sizeText, excludeOrderId = null }) {
  ...
  for (const oi of outItems) {
    const ord = oi.order;
    if (!ord) continue;
    const returnBy = ord.toDate || ord.returnDate || (ord.eventDate ? addDaysSkippingWeekends(ord.eventDate, 1) : null);
    if (!returnBy) continue;
    const key = new Date(returnBy).toISOString().split('T')[0];
    if (key === tomorrowKey) {
...
export async function validateOrderItemsAvailability(items, eventDate, isAbroad, fromDate, toDate, orderId = null, customSpacing = null) {
  ...
  // B5: הרצה מקבילית במקום לולאה סדרתית — חוסך 15 סבבים DB להזמנה עם 5 דגמים
  const availabilities = await Promise.all(modelsToCheck.map(modelId => getAvailableInventory(
    modelId,
    targetMinDate,
    bufferDays,
    skipWeekends,
  ...
  for (let idx = 0; idx < modelsToCheck.length; idx++) {
    const modelId = modelsToCheck[idx];
