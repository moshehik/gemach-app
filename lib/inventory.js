...
// docs/fix-protocol-error-reports.md #15 - reverse-engineered from the live VBA generator,
// קוד_למחרוזת/חישוב_ימים_לפני_ואחרי in access_code/Modules/הזמנות_readable.txt, and verified
// against live Access data). o(k) is the occupancy (summed qty of OTHER bookings whose own,
// unexpanded date/interval covers that exact calendar day) at business-day offset k from the
// query date, for k in [-bufferDays, bufferDays]. This is a Kadane-style running-max delta
// chain, NOT a naive per-calendar-day max (undercounts two different bookings that both fall
...
// query date, to share one physical unit sequentially - e.g. bookings at offset -2 and +1 with
// bufferDays=2 only need 1 unit between them, but bookings at offset -2 and 0 need 2).
