...
export function computeInventoryAvailability(context) {
  ...
  // docs/fix-protocol-error-reports.md #12/#13. The former daysAfter=bufferDays+1 asymmetry was
  // unexplained and is the reason this file's numbers diverged from Access's.
  //
  // The buffer is applied on ONE side only - the target's own sweep window (targetDays below) -
  // NOT a second time to each other booking's own date. Access's real query never expands the
  // other booking: it tests whether that booking's own exact event date literally equals one of
  // the swept candidate days. Applying the buffer on both sides (as this file did before
  // 2026-09-15) doubles the effective gap enforced between two different customers' bookings to
  // 2*bufferDays instead of bufferDays - confirmed as a real bug (not just a difference from
  // legacy Access behavior) by re-testing model 549 order-by-order: e.g. order 52733 (event
  // 12/10) was wrongly counted as blocking a 07/10 query because ITS OWN ±bufferDays window
  // reached back into the target's ±bufferDays window, even though the two dates are
  // 2*bufferDays apart, not bufferDays apart. See docs/fix-protocol-error-reports.md #13.
    ...
    for (const dTime of targetDays) {
      ...
      for (const booking of bookings) {
        ...
        if (customStart && customEnd) {
          ...
          occupiesDay = (dTime === bookingEventDate.getTime());
        }
...
export async function getBulkAvailableInventory(targetDate, modelIds = null) {
  ...
  // targetDays below, not a second time to each other booking's own date - see the long
  // comment above computeInventoryAvailability's identical daysAfter/daysBefore for the full
  // explanation (docs/fix-protocol-error-reports.md #13).
