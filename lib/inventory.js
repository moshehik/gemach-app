...
export async function getAvailableInventory(
  ...
  if (ignoreOrderId) {
    bookingsWhere.order.id = { not: ignoreOrderId };
  }
...
export async function validateOrderItemsAvailability(items, eventDate, isAbroad, fromDate, toDate, orderId = null) {
  ...
  if (isAbroad) {
    if (!fromDate || !toDate) {
       return { valid: false, error: 'חסרים תאריכים עבור אירוע חו"ל' };
    }
    targetMinDate = fromDate;
    targetMaxDate = toDate;
  } else {
    if (!eventDate) {
       return { valid: false, error: 'חסר תאריך אירוע' };
    }
