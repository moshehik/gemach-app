 ...
 */
// `now` is optional and only used by the admin refund tools (/admin/refund-planner,
// /admin/refund-simulator) to evaluate the same rules "as of" another moment. Every
// production caller omits it, so behaviour there is unchanged: `currentDate` is real time.
export function computeOrderObligations({ order, items, deletedItems, priceList, settings, customNote = null, now = null }) {
...
/**
 * Pure calculation of the delivery obligation a `PUT /api/orders/[id]` would create via
 * `applyDeliveryCharge` (lib/pricingEngine.js) - same price-lookup rules (per-city table,
 * falling back to a flat price, falling back to the documented 50 ILS default), but without
 * touching the DB. Used by the preview-pricing route so an accurate total (including
 * delivery, which `computeOrderObligations` above has no notion of) can be shown to an
 * employee BEFORE they actually save - `applyDeliveryCharge` itself stays the write path
 * and does not call this to avoid a needless second settings/price-map parse on the hot path.
 *
 * Returns null when there's nothing to charge (not a delivery order, no city selected, or an
 * existing non-deleted "משלוח" obligation already accounts for it - matching
 * `applyDeliveryCharge`'s own "don't double-charge / don't update an existing charge" rule).
 */
export function computeDeliveryObligationPreview({
  isDelivery,
  deliveryCity,
  deliveryDirection,
  deliveryPriceByCity, // raw `delivery_price_by_city` SystemSetting value (JSON string or falsy)
  deliveryPrice, // raw `delivery_price` SystemSetting value (string or falsy)
  existingObligations = []
}) {
  if (!isDelivery || !deliveryCity) return null;

  const alreadyCharged = (existingObligations || []).some(o =>
    !o.isDeleted && String(o.description || '').includes('משלוח')
  );
  if (alreadyCharged) return null;

  let priceMap = {};
  try { priceMap = JSON.parse(deliveryPriceByCity || '{}'); } catch { /* ignore malformed setting */ }
  let cityPrice = priceMap[deliveryCity];
  if (!cityPrice || Number(cityPrice) <= 0) cityPrice = deliveryPrice;
  if (!cityPrice || Number(cityPrice) <= 0) cityPrice = 50;

  const dir = deliveryDirection || 'הלוך-חזור';
  const count = dir === 'הלוך-חזור' ? 2 : 1;
  const amount = Number(cityPrice) * count;

  return {
    amount,
    quantity: count,
    description: `משלוח ${dir} - ${deliveryCity}`,
    isManual: false
  };
}
