...
function anchorPlusDaysIso(anchor, days) {
...
export async function getDeliveriesForDate(requestedDate) {
  ...
  const settingsRows = allSettings.filter(s => ['delivery_days_before', 'delivery_days_after', 'deliveries_select_by_event_date'].includes(s.key));
  ...
  for (const order of orders) {
    ...
    if (selectByEventDate) {
      ...
      if (orderDirection !== 'חזור') {
        directions.push('out');
        dispatchDates.out = anchorPlusDaysIso(eventAnchor, -(order.deliveryOneDayBefore ? 1 : daysBefore));
      }
      if (orderDirection !== 'הלוך') {
        directions.push('return');
        dispatchDates.return = anchorPlusDaysIso(eventAnchor, daysAfter);
      }
    } else {
