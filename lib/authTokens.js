...
//     employeeId + roleId (+ a legacy `a` flag nobody reads any more), issued at login alongside the legacy
...
// Payload:       { e: employeeId, r: roleId, a: false (legacy, unused), iat: ms, exp: ms }
//
...
const SESSION_FRESH_MS = 15 * 60 * 1000; // role trusted without DB for 15 min
...
// THE identity gate for every server-side read of the `auth_token` cookie (which is just the
// raw employee UUID — anyone can type one into their browser, and the UUIDs are listed by the
// public login-picker endpoint GET /api/employees). Returns that cookie ONLY when a signed,
// unexpired `auth_session` cookie issued at a real login (password / trusted-device PIN) names
// the same employee; otherwise undefined, i.e. the request is treated as anonymous. Every place
// that used to call cookieStore.get('auth_token') now goes through this, so a hand-forged
// auth_token can no longer impersonate anyone (found + fixed 2026-09-20, see CLAUDE.md
// "Auth cookie forgery"). When AUTH_SECRET is not configured at all we cannot verify anything
// and fall back to the raw cookie (legacy behaviour) rather than lock everybody out — both live
// deployments have the secret set (verified: login issues auth_session).
function getVerifiedAuthCookie(cookieStore, secret = process.env.AUTH_SECRET || null, now = Date.now()) {
  const raw = cookieStore && cookieStore.get(AUTH_COOKIE_NAME);
  if (!raw || !raw.value) return undefined;
  if (!secret) return raw;
  const session = verifySessionToken(cookieStore.get(SESSION_COOKIE_NAME)?.value, secret, now);
  if (!session || session.e !== raw.value) return undefined;
  return raw;
}

// A cookie store whose auth_token reads are verified — for helpers that take a cookieStore and
// call .get('auth_token') themselves (lib/ai/aiCommon.js loadEmployeeAccess) and must stay
// dependency-free.
function verifiedCookieStore(cookieStore, secret = process.env.AUTH_SECRET || null) {
  return {
    get: (name) => (name === AUTH_COOKIE_NAME ? getVerifiedAuthCookie(cookieStore, secret) : cookieStore.get(name)),
  };
}
...
// Mirrors the original checkPageAccess() flow (manager-only page guard by
// default). `allowedRoles` lets a caller tighten the gate (e.g. [0, 2] for a
// הנהלה ראשית-only page) without touching the default [1, 2] behavior every
...
async function checkPageAccessCore({
  ...
  if (authTokenValue) {
    // Fast path — only ever used to GRANT access, never to deny.
    const session = verifySessionToken(sessionTokenValue, secret, now);
    if (
      session &&
      session.e === authTokenValue &&
    ...
    if (employee) {
      return allowedRoles.includes(employee.roleId);
    }
