...
// `SELECT * FROM "Employee"` never names password/pinHash but still returns them - the
// identifier regex above can't see it, so callers who allow `*` (unlike checkReadOnlySelect's
// callers, which run stripSecretColumns on the result instead) need this dedicated check too.
const EMPLOYEE_WILDCARD_REGEX = /\bSELECT\s+\*\s+FROM\s+"?Employee"?\b/i;

/**
 * Narrower than checkReadOnlySelect: only checks for credential-column/table exposure,
 * without rejecting mutating statements. For callers (like the admin SQL generator) that
 * intentionally allow the AI to write INSERT/UPDATE/DELETE for a trusted admin, but must
 * never let it generate a query touching Employee.password/pinHash/TrustedDevice, even via
 * an unqualified `SELECT *`.
 *
 * @param {string} sql
 * @returns {{ ok: true } | { ok: false, reason: string }}
 */
function checkNoCredentialExposure(sql) {
  if (typeof sql !== 'string' || !sql.trim()) {
    return { ok: false, reason: 'Empty or invalid SQL string' };
  }
  if (SENSITIVE_IDENTIFIER_REGEX.test(sql)) {
    return { ok: false, reason: 'Query references a protected credential column/table (password/pinHash/TrustedDevice)' };
  }
  if (EMPLOYEE_WILDCARD_REGEX.test(sql)) {
    return { ok: false, reason: 'Query selects * from Employee, which would silently include password/pinHash - list the specific columns needed instead' };
  }
  return { ok: true };
}

export { assertReadOnlySelect, checkReadOnlySelect, checkNoCredentialExposure, stripSecretColumns };
