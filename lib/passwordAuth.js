...
/**
 * True if `value` is shaped like a bcrypt hash ($2a$/$2b$/$2y$ prefix). Used to tell an
 * already-hashed Employee.password apart from a legacy plaintext one (see app/api/login -
 * many employees imported from the old Access system carry their original short numeric
 * code as plaintext in this column, never hashed).
 */
