...
export async function findEmployeeCandidatesByUsername(username, { activeOnly = true } = {}) {
  if (!username) return [];
  const parsedLegacy = /^\d+$/.test(String(username)) ? parseInt(username, 10) : NaN; // digits only: a UUID that merely STARTS with digits must not match some other employee's legacyId
