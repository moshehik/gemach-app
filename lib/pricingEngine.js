...
export function computeOrderObligations({ order, items, deletedItems, priceList, settings, customNote = null }) {
  ...
  for (const delItem of sortedDeletedItems) {
    ...
    if (delItem.createdAt && delItem.deletedAt && !isNaN(cancellationCreditMinutes)) {
      const lifetimeMs = new Date(delItem.deletedAt).getTime() - new Date(delItem.createdAt).getTime();
    ...
    if (isCreditEligible) {
      ...
      if (consumedCredit > 0) {
        newObligations.push({
          ...
          quantity: delItem.quantity || 1,
          description: 'זיכוי דמי ביטול (מומש על פריט חדש): ' + delItem.dressItem.dress.name,
          isManual: false,
          orderItemId: delItem.id,
          isDraft: false
        });
