...
export function computeOrderObligations({ order, items, deletedItems, priceList, settings, customNote = null }) {
  ...
  for (const item of items) {
    ...
    if (premiumEnabled && isPremiumItem) {
      ...
      // אם קטגוריית הדגם עצמה נחשבת פרימיום - נשארים בה, אחרת מנסים למצוא פרימיום (לוגיקה פשוטה: מחיר לפי category הקיימת, אבל אם isPremium - מכפיל אם יש הגדרה)
      void premiumCats;
    }

    // Find matching price by checking fromSize/toSize properly
    const matchedPrice = priceList.find(p => {
      const catMatch = p.category === effectiveCategory || p.category === category || p.category === category.replace('כלול ב', '').trim();
