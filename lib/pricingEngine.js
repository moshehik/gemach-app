...
/**
 * 15 - חיוב משלוח אוטומטי לפי עיר (`delivery_price_by_city` SystemSetting).
 ...
 * No-ops when the order isn't a delivery, has no configured city price, or already has a
 * non-deleted "משלוח" obligation (matching the original creation-time create-if-missing logic
 * exactly - it does not update an existing obligation's amount if the city/price changed).
...
export async function applyDeliveryCharge(orderId) {
  try {
    ...
    const cityPrice = priceMap[order.deliveryCity];
