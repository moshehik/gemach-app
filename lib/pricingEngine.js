...
export async function recalculateOrderObligations(orderId, options = {}) {
  ...
  for (const item of order.items) {
    ...
    let desc = item.dressItem.dress.name + (item.sizeText ? ` מידה ${item.sizeText}` : '');
    ...
    if (item.neckAlteration) {
      ...
      newObligations.push({
        ...
        amount: neckCost,
        quantity: item.quantity || 1,
        description: `תיקון צוואר - ${item.dressItem.dress.name}`,
        isManual: false
      });
      totalValid += neckCost;
    }
    ...
    if (item.sleeveAlteration) {
      ...
      newObligations.push({
        ...
        amount: sleeveCost,
        quantity: item.quantity || 1,
        description: `תיקון שרוול - ${item.dressItem.dress.name}`,
        isManual: false
      });
  ...
  if (!dryRun) {
    await prisma.$transaction(async (tx) => {
      ...
      for (const item of order.items) {
        const obs = newObligations.filter(o => o.description.includes(item.dressItem.dress.name) && o.amount >= 0);
