...
export function computeOrderObligations({ order, items, deletedItems, priceList, settings, customNote = null }) {
  ...
  // החלפת מידה לאותו דגם: פריט שנמחק ופריט פעיל אחר של אותו דגם (DressModel) מתפקדים
  // כזוג "החלפה" - הלקוח/ה משלמ/ת רק את ההפרש בין המידות, בלי דמי ביטול, ללא תלות
  // בחלון הזמן/במדיניות הביטול הרגילה (ר' בקשה בדיווח 87ba8e3d). כל פריט פעיל משמש
  // כתחליף לכל היותר לפריט מחוק אחד (shift() מסיר אותו מהמאגר עם כל שיבוץ).
  ...
  for (const delItem of sortedDeletedItems) {
    ...
    // להיות חיוב קבוע כרגיל - לא נדונו בבקשה.
    const isSameModelSwap = claimSameModelReplacement(delItem);
