...
export function computeOrderObligations({ order, items, deletedItems, priceList, settings, customNote = null }) {
  ...
  for (const item of items) {
    ...
    if (premiumEnabled && isPremiumItem) {
      ...
      // כלשהי במחירון. ההתאמה המדויקת לפי מידה/תאריך מתבצעת למטה ב-priceMatches - אם היא
    ...
    // Find matching price by checking fromSize/toSize properly, for a given category
    const priceMatches = (cat) => priceList.filter(p => {
      ...
      if (order.eventDate) {
        ...
        if (p.startDate && evDate < new Date(p.startDate)) return false;
        if (p.endDate && evDate > new Date(p.endDate)) return false;
      }
      return true;
    });
    ...
    const matchedPrice =
      (effectiveCategory !== category ? priceMatches(effectiveCategory)[0] : null)
      || priceMatches(category)[0]
      || priceMatches(category.replace('כלול ב', '').trim())[0]
      || null;
  ...
  // החלפת מידה לאותו דגם (מאחורי same_model_swap_no_fee - כרגע רק נווה יעקב): פריט
  // שנמחק ופריט פעיל אחר של אותו דגם (DressModel) מתפקדים כזוג "החלפה" - הלקוח/ה
  // משלמ/ת רק את ההפרש בין המידות, בלי דמי ביטול, ללא תלות בחלון הזמן/במדיניות
  // הביטול הרגילה (ר' בקשה בדיווח 87ba8e3d). כל פריט פעיל משמש כתחליף לכל היותר
  // לפריט מחוק אחד (shift() מסיר אותו מהמאגר עם כל שיבוץ).
  // החלפת דגם (דגם שונה) ממשיכה להתנהל לפי מדיניות דמי הביטול הרגילה למטה, ללא שינוי.
  const unclaimedActiveByModel = new Map();
  for (const it of items) {
    const modelId = it.dressItem?.dress?.id;
    if (!modelId) continue;
    if (!unclaimedActiveByModel.has(modelId)) unclaimedActiveByModel.set(modelId, []);
    unclaimedActiveByModel.get(modelId).push(it);
  }
  const claimSameModelReplacement = (delItem) => {
    const modelId = delItem.dressItem?.dress?.id;
    if (!modelId) return false;
    const pool = unclaimedActiveByModel.get(modelId);
  ...
  for (const delItem of sortedDeletedItems) {
    ...
    // החלפת מידה לאותו דגם (ר' claimSameModelReplacement למעלה) - זיכוי מלא על השמלה,
    // בלי דמי ביטול, ללא תלות במדיניות/בחלון הזמן הרגילים. תיקונים (itemLost) ממשיכים
    // להיות חיוב קבוע כרגיל - לא נדונו בבקשה. פועל רק כשsame_model_swap_no_fee דלוקה -
    // ה-&& הקצר-מעגל גם מונע מ-claimSameModelReplacement "לתפוס" פריט מהמאגר כשהיא כבויה.
    const isSameModelSwap = sameModelSwapNoFee && claimSameModelReplacement(delItem);
