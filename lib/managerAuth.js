...
//
// permissionKey (אופציונלי): מפתח של פריט "מאשר" בקטלוג ההרשאות (approver:true, ר'
// lib/permissionsMetadata.js). כשניתן - מי שנחשב מאשר תקף נקבע ב-hasPermission (הנהלה ראשית/מתכנת
// תמיד -> חריגה אישית -> שורת מחלקה -> ברירת המחדל) ולא לפי roleId קשיח. בלי מפתח - ההתנהגות
// הישנה (roleId 1/2) נשמרת לקוראים שעדיין לא הועברו.
export async function verifyManagerPin(employeeId, pin, permissionKey = null) {
  if (!pin) return false;
  ...
  for (const candidate of candidates) {
    if (await verifySecret(pin, candidate.password)) {
      if (permissionKey) return hasPermission(candidate, permissionKey);
